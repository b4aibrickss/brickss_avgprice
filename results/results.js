...
function displayResults(results) {
  ...
  results.forEach((result, index) => {
    ...
    `;
    resultElement.addEventListener("click", () => toggleResult(resultElement, results));
    resultsContainer.appendChild(resultElement);
  });
  console.log("Results displayed in results page:", results);
}

function toggleResult(element, results) {
  element.classList.toggle("disabled");
  const index = element.dataset.index;
  results[index].disabled = !results[index].disabled;
...
function calculateStats(results) {
  ...
  if (filteredResults.length === 0) {
    document.getElementById("min-price").innerText = "Menor Preço: -";
    document.getElementById("max-price").innerText = "Maior Preço: -";
    document.getElementById("avg-price").innerText = "Preço Médio: -";
    return;
  }
  ...
  document.getElementById("min-price").innerText = `Menor Preço: R$ ${minPrice.toFixed(2)}`;
  document.getElementById("max-price").innerText = `Maior Preço: R$ ${maxPrice.toFixed(2)}`;
  document.getElementById("avg-price").innerText = `Preço Médio: R$ ${avgPrice.toFixed(2)}`;
}
...
function scrapeEbay() {
    ...
    console.log("Number of items found (eBay):", items.length);
    items.forEach(item => {
        ...
        if (titleElement && priceElement && linkElement && imageElement) {
            ...
            console.log("Missing element in item (eBay):", {
                titleElement,
                priceElement,
                linkElement,
                imageElement
